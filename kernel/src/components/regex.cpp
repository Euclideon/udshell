...
void Regex::CompilePattern(String pattern)
{
  ...
  if (!pCode)
    EPTHROW_ERROR(epR_Failure, "Regex compile failed: {0}", pError);
  epscope(fail) { pcre_free((pcre*)pCode); pCode = nullptr; };

  pExtra = pcre_study((pcre*)pCode, 0, &pError);
  if (!pExtra)
    EPTHROW_ERROR(epR_Failure, "Optimising regex failed: {0}", pError);
}

Array<String> Regex::Match(String text)
{
  EPTHROW_IF(!pCode || !pExtra, epR_InvalidCall, "Call to match requires a valid regex pattern!");
  ...
  if (matches < 0)
  {
    switch (matches)
    {
      case PCRE_ERROR_NOMATCH: // no match
        return nullptr;
      case PCRE_ERROR_NULL:         EPTHROW_ERROR(epR_InvalidArgument, "Something was null\n");       break;
      case PCRE_ERROR_BADOPTION:    EPTHROW_ERROR(epR_InvalidArgument, "A bad option was passed\n");  break;
      case PCRE_ERROR_BADMAGIC:
      case PCRE_ERROR_UNKNOWN_NODE: EPTHROW_ERROR(epR_Failure, "Error in compiled regex");            break;
      case PCRE_ERROR_NOMEMORY:     EPTHROW_ERROR(epR_AllocFailure, "Ran out of memory");             break;
      default:                      EPTHROW_ERROR(epR_Failure, "Unknown error");                      break;
    }
  }
  if (matches == 0)
  {
