...
class udLuaDelegate : public udDelegateMemento
{
protected:
  template<typename T>
  ...
  udLuaDelegate(lua_State *L, int idx)
  {
    this->L = L;
    ...
    FastDelegate<udVariant(udSlice<udVariant>)> shim(this, &udLuaDelegate::call);
    m = shim.GetMemento();
  }
...
{
  typedef udSharedPtr<udLuaDelegate> LuaDelegateRef;
