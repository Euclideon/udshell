...
//  importer.SetPropertyInteger(AI_CONFIG_PP_ICL_PTCACHE_SIZE, postTransformCacheSize?);
  ...
  for (uint32_t i = 0; i<pScene->mNumMaterials; ++i)
  {
    ...
    // add resource
    udMutableString64 buffer; buffer.sprintf("material%d", i);
...
//    spNode->GetMetadata()->Insert("parent", FromAIString(node.mParent->mName));
  ...
  for (uint32_t i = 0; i<node.mNumMeshes; ++i)
  {
    ...
    // get material
    udMutableString64 resName; resName.sprintf("material%d", mesh.mMaterialIndex);
    ...
    resName.sprintf("positions%d", numMeshes);
    ...
    if (mesh.HasNormals())
    {
      ...
      resName.sprintf("normals%d", numMeshes);
    ...
    if (mesh.HasTangentsAndBitangents())
    {
      ...
      resName.sprintf("binormalstangents%d", numMeshes);
    ...
    for (uint32_t t = 0; t<mesh.GetNumUVChannels(); ++t)
    {
      ...
      resName.sprintf("uvs%d_%d", numMeshes, t);
      resources.Insert(resName, spUVs);

      spMesh->SetVertexArray(spUVs, { udSharedString::format("a_uv%d", t) });
    }
    ...
    for (uint32_t c = 0; c<mesh.GetNumColorChannels(); ++c)
    {
      ...
      resName.sprintf("colors%d_%d", numMeshes, c);
      resources.Insert(resName, spColors);

      spMesh->SetVertexArray(spColors, { udSharedString::format("a_color%d", c) });
    }
    ...
    resName.sprintf("indices%d", numMeshes);
    resources.Insert(resName, spIndices);

    spMesh->SetIndexArray(spIndices);

    // add mesh resource
    resName.sprintf("mesh%d", numMeshes++);
